...
<xsl:template match="/">
  <scanner_vulnerabilities>
    <xsl:variable name="URI" select="/Scan/@scanname"/>
      <xsl:for-each select="//section">
        <vulnerability>
          <xsl:variable name="Attack" select="current()/plugin/title"/>
          <xsl:choose>
            <xsl:when test="contains($Attack, 'Predictable Resource Location')">
          ...
          <name><xsl:value-of select="current()/plugin/title"/></name>
          <url><xsl:value-of select="current()/vulnerabilities/instance/baseurl"/></url>
          <parameter><xsl:value-of select="current()/vulnerabilities/instance/request/raw"/></parameter>
          <cookie></cookie>
          <xsl:variable name="risk" select="current()/vulnerabilities/instance/risklevel"/>
          <xsl:choose>
            <xsl:when test="$risk='None'">
              <threat>Info</threat>
            </xsl:when>
            <xsl:otherwise>
                <threat><xsl:value-of select="current()/vulnerabilities/instance/risklevel"/></threat>
              </xsl:otherwise>
          </xsl:choose>
          <score><xsl:value-of select="current()/vulnerabilities/instance/cvssbasescore"/></score>
          <severity><xsl:value-of select="current()/vulnerabilities/instance/risklevel"/></severity>
          <status>open</status>
          <opened>1</opened>
      </vulnerability>
      </xsl:for-each>
